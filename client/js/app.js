angular
    ...
    function getCurrentUser() {
      ...
      return $http({
        ...
        url: '/current_user'
          ...
          if(data && data.data && data.data.isAuthenticated) {
  ...
  .run(['$rootScope', '$state', '$location', 'AuthService', function($rootScope, $state, $location, AuthService) {
    $rootScope.$on('$stateChangeStart', function(event, next) {
      AuthService
        .getCurrentUser()
        .then(function (currentUser) {
          if(next.permissions && currentUser.permissions == 2) {
  ...
  .controller('AppCtrl', ['$rootScope', '$scope', '$http', 'Sites','KtxUsers', '$q', function($rootScope, $scope, $http, Sites,KtxUsers, $q) {
    ...
    /************************************Notify****************************/
    /**********************************************************************/
    $scope.Notify = function(title, type) {
      $.notify(title, { globalPosition: 'bottom right', autoHideDelay: 3000, className: type});
    };
  }]);
