angular
  .module('app')
  .controller('UsersCtrl', ['$scope', '$rootScope','KtxUsers', 'Sites', '$q',
    function ($scope, $rootScope, KtxUsers, Sites, $q) {
      ...
      /////////////////////////EDIT USER///////////////////////////////////
      //////////////////////////////////////////////////////////////////////
      $scope.edituser = function(id, name, fullname, permissions, sitesId) {
        $('#val_confirm_password_edit').val("");
        $('button.submit').prop('disabled', "disabled");
        $scope.nameerroredit = false;
        $('.first-name').removeClass('has-error-name');
        $scope.euser = {
          id: id,
          name: name,
          fullname: fullname,
      ...
      $scope.saveedituser = function() {
        ...
        salt = (MD5(salt));
        var pass = (MD5($scope.euser.password + salt));

        if($scope.euser.permission == 1) {
      ...
      $scope.testnameedit = function(id, value) {
        $scope.nameerroredit = false;
        $('.first-name').removeClass('has-error-name');
        $('.submit').removeClass('has-error-button');
        var tb = $('#example-datatable #row');
        for(var i = 0; i < tb.length; i++) {
          if((tb[i].cells[2].innerHTML == value) && (tb[i].cells[0].innerHTML != id)) {
            $scope.nameerroredit = true;
            $('.first-name').addClass('has-error-name');
            $('.submit').addClass('has-error-button');
            $('#form-validation-edit').keypress(function(e) {
              //Enter key
              if (e.which == 13) {
                return false;
              }
      ...
      $scope.testname = function(value) {
        $scope.nameerror = false;
        $('.first-name').removeClass('has-error-name');
        $('.submit').removeClass('has-error-button');
        var tb = $('#example-datatable #row');
        for(var i = 0; i < tb.length; i++) {
          if(tb[i].cells[2].innerHTML == value) {
            $scope.nameerror = true;
            $('.first-name').addClass('has-error-name');
            $('.submit').addClass('has-error-button');
            $('#form-validation').keypress(function(e) {
              //Enter key
              if (e.which == 13) {
                return false;
              }
        ...
        if(confirm("Are you want to delete this user ?")) {
          if(name == 'superadmin' && id == '1') {
            ...
            KtxUsers
              ...
              .$promise
              .then(function(data) {
                var index = _.findIndex($scope.listusers, function(item){
                  return item.id == id;
                });
                ...
                $scope.Notify("Deleted user success","success");
              }, function(err) {
                console.log(err);
                $scope.Notify("Error when delete user","error");
              });
          }
        }
      };
